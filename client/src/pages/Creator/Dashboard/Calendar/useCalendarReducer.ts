...
type ServerOccurrence = OccurrenceQuery & {
    bookings?: {
        _id: string;
        numGuests: number;
        bookingType: BookingType;
        client: { firstName: string; photo?: string | null; }
    }[];
}
...
const createCalendarOccurrence = (occ: ServerOccurrence) => {
    ...
    const created: Occurrence = {
        ...
        numGuests: occ.bookings && occ.bookings.length > 0 ?
            occ.bookings.map(({ numGuests }) => 
                numGuests
            ).reduce((acc, val) => acc + val) : 0,
        bookings: occ.bookings ? occ.bookings.map(booking => ({
            _id: booking._id,
